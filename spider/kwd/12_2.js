//http://www.china-tender.com.cn/jggg/index.jhtml
...
(async () => {
    ...
    //页数236
    for(let i=1; i<=236; i++) {
        let url = 'http://www.china-tender.com.cn/jggg/index.jhtml' ;
        if(i>1)
            url = 'http://www.china-tender.com.cn/jggg/index_'+i+'.jhtml';
        await page.goto(url);
        ...
        async function getData(url) {
            ...
            // await page.screenshot({path: '12_2.png'});

            let data = await detail.evaluate(() => {
                let data = {};
                let regExp1 = /\d{4}[-/]\d{2}[-/]\d{2}/; //附匹配括号中内容的正则: /(?<=\()[^\(\)]*(?=\))/
                let regExp2 =/(招标|采购|项目)编号[:,：][^)\n）]+/;
                let regExp3 = /(成 交 人：|第一名：|中标人名称：|中标候选人第 1 名：|中标候选人第一名：|第一名 |第一中标候选人\t)[\u4e00-\u9fa5]+/;
