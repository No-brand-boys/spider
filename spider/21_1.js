...
(async () => {
    ...
    var pageNum = 1;
    var url = 'https://ecp.cgnpc.com.cn/CmsNewsController.do?method=newsList&channelCode=zgh_zbgg&parentCode=zgh_cgxx&param=bulletin&rp=20&page=' + pageNum;
    ...
    do {
        ...
        async function getData(url, i) {
            ...
            let data = await detail.evaluate((url) => {
                ...
                let data = {};
                let body = $(".frameReport")[0].children[2].innerHTML.replace(/<\/?.+?\/?>/g, '').replace(/&nbsp;/g, '');
                ...
                try {
                    ...
                    data.region_type_id = '';
                    data.qualification_requirements = qualification_requirements_pattern.exec(data.body)[0];
                    return data;
                }
    ...
    browser.close();
    fs.writeFile("21_1.json", JSON.stringify(datas, null, '\t'), {flag: "w"}, function (err) {
        if (err) {
